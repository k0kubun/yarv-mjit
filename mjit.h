...
typedef VALUE (*mjit_func_t)(rb_thread_t *, rb_control_frame_t *);

/* Try to execute the current iseq in thread TH.  Use JIT code if it is ready.
   If it is not, add ISEQ to the compilation queue and return Qundef.  */
static inline VALUE
mjit_exec(rb_thread_t *th)
{
