...
fprint_opt_call_fallback(FILE *f, int insn, VALUE ci, VALUE cc, unsigned int result_pos, unsigned int argc, VALUE key)
{
    ...
    fprintf(f, "      struct rb_calling_info calling;\n");
    if (key) {
	fprintf(f, "      *(cfp->sp - %d) = recv;\n", argc);
	fprintf(f, "      *(cfp->sp - %d) = rb_str_resurrect(0x%"PRIxVALUE");\n", argc - 1, key);
	if (argc == 3) { /* for opt_aset_with */
	    fprintf(f, "      *(cfp->sp - 1) = obj;\n");
	}
    }
    else {
	fprintf(f, "      *(cfp->sp - %d) = recv;\n", argc);
	if (argc >= 2) {
	    fprintf(f, "      *(cfp->sp - %d) = obj;\n", argc - 1);
	}
	if (argc >= 3) {
	    fprintf(f, "      *(cfp->sp - 1) = obj2;\n");
	}
    }
    /* CALL_SIMPLE_METHOD */
    fprintf(f, "      calling.block_handler = VM_BLOCK_HANDLER_NONE;\n");
	     ...
	     const unsigned int pos, struct compile_status *status, struct compile_branch *b)
{
    ...
    /* Move stack pointer to keep the place to write back local variables when exception is raised */
      ...
      case BIN(expandarray):
	{
	    ...
	    for (i = 0; i < space_size; i++) { /* TODO: use memmove? */
		fprintf(f, "  stack[%d] = *(cfp->sp - %d);\n", b->stack_size + space_size - 1 - i, i + 1);
	    }
